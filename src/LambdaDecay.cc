...
void LambdaDecay::Generate(TLorentzVector& lv_lambda,TVector3& vec_lambda_spin_direction)
{
  ...
  if(vec_proton_spin_direction_.Mag()==0.){
    ...
    vec_lambda_spin_direction_ = TVector3(sin(theta_spin)*cos(phi_spin),sin(theta_spin)*sin(phi_spin),cos(theta_spin));
  }
  ...
  while(true){
    ...
    Double_t val_random = random_->Uniform(0.,KppGeneratorCommon::kMaxRatioDecay);
    if(val_ratio>val_random){
      break;
    }
  }

  Double_t momentum_decay = sqrt( (lv_lambda_.M2()-(KppGeneratorCommon::kProtonMass+KppGeneratorCommon::kPiMass)*(KppGeneratorCommon::kProtonMass+KppGeneratorCommon::kPiMass))*(lv_lambda_.M2()-(KppGeneratorCommon::kProtonMass-KppGeneratorCommon::kPiMass)*(KppGeneratorCommon::kProtonMass-KppGeneratorCommon::kPiMass)) )/2./lv_lambda_.M();
  ...
  lv_pim_.SetVectM(vec_pim,KppGeneratorCommon::kPiMass);
