...
void EventAction::EndOfEventAction(const G4Event* event)
{
  ...
  for(int i_dc=0; i_dc<Driftchamber::kTotalNumber; ++i_dc){
    auto dc_hitscollection = GetHC(event,driftchamber_hitscollection_ids_[i_dc]);
    auto number_of_hits_in_dc = (int)dc_hitscollection->GetSize();
    rootfile_manager.FillNtupleIColumn("tree_event",Form("number_of_hits_in_%s",Driftchamber::kDetectorNames[i_dc].data()),number_of_hits_in_dc);
    for(G4int i_hit=0; i_hit<number_of_hits_in_dc; ++i_hit){
      DriftChamberHit* hit = (DriftChamberHit*)dc_hitscollection->GetHit(i_hit);
      if(hit){
        rootfile_manager.FillNtupleIColumn(Form("tree_%s_hit",Driftchamber::kDetectorNames[i_dc].data()),"event_id",event_id);
        // -----
        int track_id = hit->GetTrackID();
        rootfile_manager.FillNtupleIColumn(Form("tree_%s_hit",Driftchamber::kDetectorNames[i_dc].data()),"track_id",track_id);
        // -----
        int parent_id = hit->GetParentID();
        rootfile_manager.FillNtupleIColumn(Form("tree_%s_hit",Driftchamber::kDetectorNames[i_dc].data()),"parent_id",parent_id);
        // -----
        int particle_id = hit->GetParticleID();
        rootfile_manager.FillNtupleIColumn(Form("tree_%s_hit",Driftchamber::kDetectorNames[i_dc].data()),"particle_id",particle_id);
        // -----
        int layer_id = hit->GetLayerID();
        rootfile_manager.FillNtupleIColumn(Form("tree_%s_hit",Driftchamber::kDetectorNames[i_dc].data()),"layer_id",layer_id);
        // -----
        float hit_time = (float)hit->GetHitTime();
        rootfile_manager.FillNtupleFColumn(Form("tree_%s_hit",Driftchamber::kDetectorNames[i_dc].data()),"hit_time",hit_time);
        // -----
        G4ThreeVector hit_position = hit->GetGlobalPosition();
        rootfile_manager.FillNtupleFColumn(Form("tree_%s_hit",Driftchamber::kDetectorNames[i_dc].data()),"hit_position_x",(float)hit_position.x());
        rootfile_manager.FillNtupleFColumn(Form("tree_%s_hit",Driftchamber::kDetectorNames[i_dc].data()),"hit_position_y",(float)hit_position.y());
        rootfile_manager.FillNtupleFColumn(Form("tree_%s_hit",Driftchamber::kDetectorNames[i_dc].data()),"hit_position_z",(float)hit_position.z());
        // -----
        G4ThreeVector momentum = hit->GetMomentum();
        rootfile_manager.FillNtupleFColumn(Form("tree_%s_hit",Driftchamber::kDetectorNames[i_dc].data()),"momentum_x",(float)momentum.x());
        rootfile_manager.FillNtupleFColumn(Form("tree_%s_hit",Driftchamber::kDetectorNames[i_dc].data()),"momentum_y",(float)momentum.y());
        rootfile_manager.FillNtupleFColumn(Form("tree_%s_hit",Driftchamber::kDetectorNames[i_dc].data()),"momentum_z",(float)momentum.z());
        // -----
        int is_asymmetric_scattering = (int)hit->IsAsymmetricScattering();
        rootfile_manager.FillNtupleIColumn(Form("tree_%s_hit",Driftchamber::kDetectorNames[i_dc].data()),"is_asymmetric_scattering",is_asymmetric_scattering);
        // -----
        rootfile_manager.AddNtupleRow(Form("tree_%s_hit",Driftchamber::kDetectorNames[i_dc].data()));
      }
    }
  }
